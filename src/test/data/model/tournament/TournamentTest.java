...
public class TournamentTest {
	...
	public void solveTest() {
		tournament = new Tournament("Tournament with 2 solutions", new Event(
			"Event",
		...
		try {
			...
			Map<Event, EventSchedule> schedules = new HashMap<Event, EventSchedule>(tournament.getCurrentSchedules());
			...
			for (Event e : tournament.getCurrentSchedules().keySet())
				assertNotEquals(
					schedules.get(e).getMatches().get(0).getStartTimeslot(),
	...
	public void groupEventsByNumberOfPlayersPerMatchTest() {
		...
		Tournament tournament = new Tournament("Tournament", new Event(
				"E1",
				new Random().ints(0, players.size()).distinct().limit(16).mapToObj(i -> players.get(i)).collect(Collectors.toList()),
				localizations,
				timeslots,
				1, 2, 2
			), new Event(
				...
				"E3",
				new Random().ints(0, players.size()).distinct().limit(32).mapToObj(i -> players.get(i)).collect(Collectors.toList()),
				localizations,
				timeslots,
				1, 2, 4
			), new Event(
				"E3",
				new Random().ints(0, players.size()).distinct().limit(36).mapToObj(i -> players.get(i)).collect(Collectors.toList()),
				localizations,
				timeslots,
				1, 2, 6
			)
	...
	public void breaksTest() {
		...
		t.addBreaks(new HashSet<Timeslot>(Arrays.asList(timeslots.get(10), timeslots.get(11))));
		...
		t.addBreak(timeslots.get(6));
		assertEquals(1, e1.getBreaks().size());
		t.addBreaks(new HashSet<Timeslot>(Arrays.asList(timeslots.get(6))));
		assertEquals(1, e1.getBreaks().size());
